...
Configure static file handling for the build output. These options apply to both `server` and `handler` build targets.

#### build.public.copy

Whether to copy the public directory into the output directory during build. Defaults to `false`.

When `true`, the contents of the source directory (set by `from`) are copied into the destination directory (set by `to`) inside `outputDir`.

#### build.public.from

The source directory path for static files. Defaults to `"./public"`.

#### build.public.to

The destination directory for the public files inside the output directory. Defaults to the value of `from`.
