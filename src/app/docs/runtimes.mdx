...
When using [environment variables](/docs/environment-variables) with the `workerd` runtime, srvkit automatically injects `import { env } from "cloudflare:workers"` into the output entry.
